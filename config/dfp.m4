...
[
  case $1 in
    aarch64*-*-darwin*)
      # Match the i386/x86_64-apple-darwin precedent: Darwin has no DFP
      # runtime in libgcc (no t-dfprules, no libbid arithmetic is built),
      # so decimal float is not supported here.  Listed before the broad
      # aarch64* case so aarch64-linux etc. keep DFP enabled.
